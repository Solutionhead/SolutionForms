var _each = require('lodash/map'),
  ...
  moment = require('koExtenders/knockout.extenders.moment'),
  moment = require('koExtenders/knockout.extenders.moment'),
  moment = require('bindings/datepicker');
...
function ScheduleSimpleView(params) {
  ...
  self.renderTeamHeader = function renerTeamHeader(appointment) {
...
ScheduleSimpleView.prototype.fetchEvents = function () {
  ...
  var q = "StartDate:[" + beginningDateRange + " TO " + endingDateRange + "]"
    +  "OR (" + recurringEventIsInRange(endingDateRange) + " AND " + eventIsOnTheCorrectDay(self.date.format("dddd"))
    +")";
  console.log(q);
...
function EventInstance(values) {
  ...
  var self = this,
    mDate = moment(values.date),
    fDate = mDate.format('MM/DD/YYYY');
